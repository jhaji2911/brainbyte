...
export const InteractiveLesson: React.FC<Props> = ({ onComplete, onNavigate }) => {
  ...
  const handlePressIn = () => {
    ...
    Animated.parallel([
      Animated.spring(buttonScale, { toValue: 0.92, friction: 5, useNativeDriver: true }),
      Animated.timing(buttonGlow, { toValue: 1, duration: 150, useNativeDriver: false }),
    ]).start();
  };
  ...
  const handlePressOut = () => {
    ...
    Haptics.impactAsync(
      char === '-' ? Haptics.ImpactFeedbackStyle.Medium : Haptics.ImpactFeedbackStyle.Light
    );

    Animated.parallel([
      Animated.spring(buttonScale, { toValue: 1, friction: 5, useNativeDriver: true }),
      Animated.timing(buttonGlow, { toValue: 0, duration: 200, useNativeDriver: false }),
    ]).start();

    const targetCode = MORSE_MAP[TARGET_WORD[currentLetterIndex]];
    const nextInput = [...currentInput, char];

    const isCorrectSoFar = nextInput.every((val, idx) => val === targetCode[idx]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <Text style={styles.streakBadge}>🔥 12</Text>
          <TouchableOpacity>
      ...
      >
        ...
        <View style={styles.tipCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.tipSubtitle}>
              Tap short for dots (.), hold long for dashes (-). Master the rhythm to unlock the
              secret language of the wire.
            </Text>
          </View>
        </View>
            ...
            {TARGET_WORD.split('').map((char, i) => (
              <Text
                ...
                style={[
                  ...
                  i > currentLetterIndex && { color: COLORS.onSurfaceVariant, opacity: 0.3 },
                ]}
              >
                {char}
              </Text>
        ...
        <View style={styles.tapArea}>
          <Animated.View style={[styles.tapGlow, { backgroundColor: glowColor }]} />
          <Animated.View style={{ transform: [{ scale: buttonScale }] }}>
            ...
            >
              <MaterialCommunityIcons name="hand-pointing-up" size={48} color={COLORS.white} />
...
const LegendChip = ({
  ...
  >
    <Text style={[legendStyles.char, active && { color: COLORS.primary }]}>{char}</Text>
    <Text style={[legendStyles.code, active && { color: COLORS.primary }]}>{code}</Text>
  </View>
);
