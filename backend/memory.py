...
class MemoryDB:
    """SQLite + NumPy hybrid memory for the curation agent.

    Three memory types:
    - episodic: user timeline of interactions
    - semantic: extracted knowledge about users and content
    - procedural: curation patterns that worked (exemplars)

    Content is generated on-the-fly by the LLM curator and stored here.
    """
    ...
    def _init_schema(self):
        ...
        # Content (knowledge bytes — LLM-curated on demand)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS content (
                ...
                source TEXT,
                format_type TEXT DEFAULT 'fact',
                created_at TEXT DEFAULT (datetime('now')),
                engagement_count INTEGER DEFAULT 0,
                avg_rating REAL DEFAULT 0.0
            )
        """)
        # Add format_type column if upgrading from older schema
        try:
            cur.execute(
                "ALTER TABLE content ADD COLUMN format_type TEXT DEFAULT 'fact'"
            )
        except Exception:
            pass
    ...
    def add_curated_content(
        ...
        """Store an LLM-curated content byte."""
        self.conn.execute(
            """INSERT OR REPLACE INTO content
               (id, title, body, category, tags, difficulty, source, format_type)
               VALUES (?,?,?,?,?,?,?,?)""",
            (
                content_id,
                title,
